...
# 0.1.1
## Main control script
const WALKER = preload("res://Examples/0_1_1/walker_adv.tscn")
var spawn_num: int = 1
...
@onready var x_slider = $CanvasLayer/GUI/MarginContainer/VBoxContainer/HBoxContainer6/XSlider
@onready var y_slider = $CanvasLayer/GUI/MarginContainer/VBoxContainer/HBoxContainer7/YSlider
@onready var x_viz = $CanvasLayer/GUI/MarginContainer/VBoxContainer/HBoxContainer6/Viz
@onready var y_viz = $CanvasLayer/GUI/MarginContainer/VBoxContainer/HBoxContainer7/Viz
...
func _ready():
	for i in range(1):
		...
		w.set_values(walk_speed_value, draw_step_value, line_lifetime_value, walker_step_time_max, x_bias, y_bias)
...
func _on_add_button_button_up():
	for i in range(spawn_num):
		var w = WALKER.instantiate()
		w.set_position(Vector2(randi_range(100,1000), randi_range(100, 1000)))
		add_child(w)
		w.set_values(walk_speed_value, draw_step_value, line_lifetime_value, walker_step_time_max, x_bias, y_bias)
## ------------------------------------------------- #
...
func _on_x_slider_drag_ended(value_changed):
	if value_changed:
		x_bias = x_slider.value
		x_viz.text = str(x_bias)
		get_tree().call_group("walkers", "update_xy_bias", x_bias, y_bias)

func _on_y_slider_drag_ended(value_changed):
	if value_changed:
		y_bias = y_slider.value
		y_viz.text = str(y_bias)
		get_tree().call_group("walkers", "update_xy_bias", x_bias, y_bias)
